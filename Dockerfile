...
RUN echo "install.packages('devtools',repos='https://cloud.r-project.org');"  > /tmp/install.R && \
    ...
    echo "install.packages(c('rdbnomics','eurostat','ggrepel','ggraph','ggiraph','ggnetwork','ggTimeSeries','plotrix','tmap','rsdmx','leaflet','shinyjs'),repos='https://cloud.r-project.org')" >> /tmp/install.R && \
